...
# --- DEBUG BANNER ---
st.info("🚀 RUNNING VERSION: v147.0 (UX Overhaul: Reduced Scrolling + Fixed Startup Error)")
...
def theta_decay_model(initial_theta, days_held, strategy, dte_at_entry=45):
    """
    Different strategies decay differently:
    - Naked options: Exponential (t^-0.5)
    - Credit Spreads: Linear then accelerates
    - Butterflies/ICs: Parabolic (max at 0.5 DTE)
    """
    t_frac = min(1.0, days_held / dte_at_entry) if dte_at_entry > 0 else 1.0
        
    # Strategy-specific models
    if strategy in ['M200', '130/160', '160/190', 'SMSF']:  # Income spreads / Butterflies
        # Parabolic: Theta peaks at 50% of DTE, then collapses
        if t_frac < 0.5:
            # Growing phase (opening to mid-life)
            decay_factor = 1 - (2 * t_frac) ** 2
        else:
            # Collapse phase (mid-life to expiration)
        ...
        # Linear decay until last 2 weeks, then exponential
        if t_frac < 0.7:
    ...
    else:  # Default: Moderate exponential
...
def reconstruct_daily_pnl(trades_df):
    ...
    for _, trade in trades.iterrows():
        ...
        initial_theta = trade['Theta'] if trade['Theta'] != 0 else 1.0 # Avoid div by zero
        
        # Generate expected theta curve
        daily_theta_weights = []
        for day in range(days):
            expected_theta = theta_decay_model(
                initial_theta, day, strategy, max(45, days) # Approx DTE
            )
            daily_theta_weights.append(abs(expected_theta)) # Use abs to handle potential negative values gracefully for weights
...
def load_data():
    empty_schema = pd.DataFrame(columns=[
        ...
        'Theta/Cap %', 'Ticker', 'Stability', 'Grade', 'Reason'
    ])
...
# --- v147.0: MOVED CALCULATOR TO SIDEBAR ---
with st.sidebar.expander("🧮 Pre-Flight Calculator", expanded=False):
    ...
    pf_price = st.number_input("Net Price ($)", value=5000.0, step=100.0)
    ...
    if st.button("Run Check", key="pf_calc_side"):
        st.markdown("---")
        if "Hedged Income" in pf_goal:
            ...
            else: st.info(f" Ann. ROI: {annualized_roi:.0f}%")
            ...
            st.metric("Leverage", f"{leverage:.2f} /$100")
            ...
            st.metric("Vega Exposure", f"{vega_efficiency:.1f}%")
            st.info(f"Need {move_needed:.1f}% IV move to break even")
...
with tab_dash:
    if not df.empty and 'Status' in df.columns:
        ...
        if active_df.empty:
            ...
            # --- v147.0: NEW SUB-TABS LAYOUT ---
            d_t1, d_t2, d_t3 = st.tabs(["📋 Actions & Journal", "🗺️ Heatmap & Visuals", "🔎 Strategy Detail"])
            
            with d_t1:
                # 1. PRIORITY QUEUE
                if not todo_df.empty:
                    st.warning(f"🚨 Priority Actions ({len(todo_df)})")
                    for _, row in todo_df.iterrows():
                        ...
                        if row['Juice Type'] == 'Recovery Days': c_c.metric("Break Even", f"{row['Juice Val']:.0f}d", delta_color="inverse")
                        else: c_c.metric("Left in Tank", f"${row['Juice Val']:.0f}")
                else: st.success("✅ No critical actions required.")
                
                st.markdown("###  Active Journal")
            ...
            with d_t3:
                ...
                strat_tabs_inner = st.tabs(["Overview"] + sorted_strats + ["Other"])
                ...
                for i, strat_name in enumerate(sorted_strats):
                    with strat_tabs_inner[i+1]:
                        ...
                        if not subset.empty:
                            st.dataframe(subset[cols], use_container_width=True, column_config={"Link": st.column_config.LinkColumn("OS Link", display_text="Open "), "Urgency Score": st.column_config.ProgressColumn("Urgency", min_value=0, max_value=100, format="%d"), "Gauge": st.column_config.TextColumn("Tank / Recov")})
...
with tab_analytics:
    # --- v147.0: CONSOLIDATED ANALYTICS ---
    an_view = st.selectbox("Select Analysis View", ["Performance & Risk", "Trends & Seasonality", "Trade Decay & DNA", "Roll Campaigns"])
    
    if an_view == "Performance & Risk":
        if not df.empty:
            active_df = df[df['Status'].isin(['Active', 'Missing'])].copy()
            ...
            try:
                if not expired_df.empty:
                    ...
                    if not snaps.empty:
                        ...
                        viz_mae = merged_mae[merged_mae['Status'] == 'Expired'].copy()
                        
                        if not viz_mae.empty:
                            mae_c1, mae_c2 = st.columns(2)
                            with mae_c1:
                                fig_mae_scat = px.scatter(viz_mae, x='MAE', y='P&L', color='Strategy', hover_data=['Name', 'Days Held'], title="Drawdown (MAE) vs Final P&L")
    ...
    elif an_view == "Trends & Seasonality":
...
with tab_ai:
    ...
    if df.empty or expired_df.empty:
        ...
        if not ai_view_df.empty:
            # Pass total_cap to prediction engine for Kelly sizing
            ...
            if not preds.empty:
                ...
                with c_p1:
                    fig_pred = px.scatter(
                        ...
                        color="Confidence",
                        size="Rec. Size ($)",
                        hover_data=["Trade Name", "Strategy", "Kelly Size %"],
                        color_continuous_scale="RdYlGn",
                        title="Risk/Reward Map (Size = Kelly Rec)"
                    )
                    # Add quadrants
                ...
                with c_p2:
                    st.dataframe(
                        preds.style.format({
                            'Win Prob %': "{:.1f}%", 'Expected PnL': "${:,.0f}", 
                            'Confidence': "{:.0f}%", 'Kelly Size %': "{:.1f}%", 'Rec. Size ($)': "${:,.0f}"
